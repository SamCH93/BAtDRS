...
\caption{Marginal posterior distribution of heterogeneity variance $\tau^{2}$
  ...
  $\Sexpr{priorsd}$ is assigned to $\tau$.}
...
plotDF1$facetlab <- paste0('tau ~ "~" ~ "HN(" *', priorsd,'* ")"')
...
\caption{Probability of replication success as a function of relative sample
  ...
  $\Sexpr{round(priorsd, 2)}$ assigned to $\tau$. Replication success is defined
...
normal prior with is assigned to the heterogeneity $tau$ which is a typical
...
$\Sexpr{round(priorsd, 2)}$ so that a range of heterogeneity values around the
value of $\tau = \Sexpr{round(priorsd, 2)}$ elicited in the main manuscript is
taken into account. We see that the probability of replication success is
slightly higher but similar as for a fixed $\tau^{2} = 0.05$ from the main
manuscript.
