...
(Section~\ref{sec:multi}). Finally, we show SSD for all studies from the
\citet{Protzko2020} project using either a flat prior or an adaptive shrinkage
prior for the effect size (Section~\ref{sec:ssdprotzko}).
...
% convergence theorem.
This means that with a uniform prior nothing can be learned about the variance
$\tau^{2}$ which intuitively makes sense as estimation of a variance requires at
least two observations. The phenomenon is illustrated in
Figure~\ref{fig:margposttau2} for the data from the ``Labels''experiment
...
\caption{Marginal posterior distribution of heterogeneity variance $\tau^{2}$
  based on data from ``Labels'' experiment \citep{Protzko2020} with original
  effect estimate $\hat{\theta}_{o} = \Sexpr{round(to, 3)}$ and standard error
  $\sigma_{o} = \Sexpr{round(so, 3)}$. A
...
\begin{align*}
  ...
  &= \int_{0}^{+\infty} \Pr(\that_{r} \in S \given \that_{o}, \sigma_{o}, \sigma_{r}, \tau^{2}) \,
    f(\tau^{2} \given \that_{o}, \sigma_{o}) \, \text{d}\tau^{2}. \\
\end{align*}
This means computing the probability of replication success with a prior on
...
\caption{Probability of replication success as a function of the relative sample
  size $c = n_{r}/n_{o}$ for ``Labels'' experiment with original effect estimate
  ...
  Bayes factor at level $\gamma = 1/\Sexpr{1/levelBFr}$, fixed effect
  meta-analysis at level $\alpha = \Sexpr{sqrt(levelSigMeta)}^{2}$, effect size
...
set to zero this corresponds to the fixed effect estimate of $\theta$, while
...
\section{Fixed effect meta-analysis}
...
fixed effect meta-analysis approach replicability is assessed in terms of the
...
\begin{align*}
  ...
  &\text{and}&
  &\sigma_{m} = \left(1/\sigma^{2}_{o} + 1/\sigma^{2}_{r}\right)^{-1/2},&
\end{align*}
...
replication study and a flat initial prior for $\theta$. Fixed effect
...
$p_{m} = \Phi(\that_{m}/\sigma_{m}) \leq \alpha$ for $\that_{o} < 0$. With some
algebraic manipulations one can show that this criterion implies a success
region $S_{\text{MA}}$ for the replication effect estimate $\that_{r}$ given by
\begin{align*}
  \SMA
  =
  \begin{cases}
...
$H_{1} \colon \theta \sim \Nor(\that_{o}, \sigma^{2}_{o})$ which arises from
updating a flat initial prior for $\theta$ the original data
...
\begin{align*}
        f(\bthat_{r} \given H_{1})
        &= \int f(\bthat_{r} \given \theta) f(\theta \given H_{1})
                        \, \text{d}\theta \\
...
\begin{align*}
  \BF_{01}(\bthat_{r})
  &= \frac{f(\bthat_{r} \given H_{0})}{f(\bthat_{r} \given H_{1})}
...
\end{align*}
where $z_{o} = \that_{o}/\sigma_{o}$. One then determines the sufficiently
...
\section{Sample size determination for all studies from the replication project}
\label{sec:ssdprotzko}
Figure~\ref{fig:ssd-all} shows SSD for all studies from the \citet{Protzko2020}
replication project using a flat prior for the effect size $\theta$. As for the
illustrative subset in the main manuscript, for the majority of studies all
methods except the equivalence test require fewer samples in the replication
than in the original study.

Figure~\ref{fig:ssd-all-shrink} shows the same analyses but using an ``adaptive
shrinkage prior'' for $\theta$ (i.e., the variance of the shrinkage prior is
estimated by empirical Bayes). We see that the required sample size increases
for studies with large $p$-values compared to the analysis based on a flat prior
for $\theta$, whereas it stays roughly the same for studies with small
$p$-values. This is because studies with large $p$-values receive more shrinkage
while the shrinkage disappears with decreasing $p$-value \citep{Pawel2020}.

\begin{figure}[!htb]
<< "sample-size-all-protzko1", results = FALSE >>=
methodLabs <- c("'Two-trials rule'", "'Replication Bayes factor'",
                "'Meta-analysis'", "'Equivalence'",
                "'Skeptical' ~ italic(p) * '-value'",
                "'Skeptical Bayes factor'")
...
\begin{figure}[!htb]
<< "sample-size-all-protzko2", results = FALSE >>=
## compute sample size for all studies with significant original study
dataSSD <- protzkolong %>%
    mutate(zo = dr/sqrt(vr)) %>%
    filter(Type == "Original")
