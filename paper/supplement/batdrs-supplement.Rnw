...
replication data. For each method we derive the success region which are needed
for sample size determination of the replication study.
...
$p_{r*} = 1 - \Phi(\that_{r*}/\sigma_{r*}) \leq \alpha$. This implies a success
region for the weighted average replication effect estimate $\that_{r*}$ given
by
\begin{align*}
  S_{\text{2TR}} = \left[\zalpha \, \sigma_{r*}, \infty \right).
\end{align*}
...
$\that_{o} > 0$). This criterion implies a success region $S_{\text{MA}}$ for
the replication effect estimate $\that_{r}$ given by
\begin{align*}
...
$i \in \{o, r\}$. A $(1 - 2\alpha)$ confidence interval for the effect size
difference $\delta = \theta_{r} - \theta_{o}$ is then given by
\begin{align*}
  \that_{r} - \that_{o} \pm \zalpha \sqrt{\sigma^{2}_{r} + \sigma^{2}_{r}}
\end{align*}
...
The data model for the replication effect estimate vector
$\bthat_{r} = (\that_{r1}, \dots, \that_{rm})^{\top}$ with standard error vector
$\bsigma_{r} = (\sigma_{r1}, \dots, \sigma_{rm})^{\top}$ is then
$\bthat_{r} \given \theta \sim \Nor_{m}\{\theta \, \bone_{m}, \diag(\bsigma^{2} + \tau^{2}_{r} \, \bone_{m}\}$
where $\tau^{2}_{r}$ is the heterogeneity variance for the replication effect
sizes and $\bone_{m}$ is a vector of $m$ ones.

To determine the multisite version of the replication Bayes factor we need to
know the marginal density of the replication effect estimates
$\hat{\theta}_r \given \theta \sim \Nor_{n}\{\theta \bone_{m}, \text{diag}(\sigma_r^2 + \tau^2_{r} \bone_{m})\}$
...
\end{align*}
The marginal density under the null hypothesis $H_{0} \colon \theta = 0$ is then
obtained by setting $m = 0$ and $v = 0$, whereas the marginal
density under the alternative hypothesis is obtained by setting $m = \that_{o}$
and $v = \sigma^{2}_{o}$. This then leads to the replication Bayes factor
\begin{align*}
      \BF_{01}(\hat{\theta}_{r})
...
\end{align*}
which is equivalent to the unisite replication Bayes factor from~\eqref{eq:bfr}
using the weighted average $\that_{r*}$ and its standard error $\sigma_{r*}$ as
the replication effect estimate $\that_{r}$ and standard error $\sigma_{r}$.
