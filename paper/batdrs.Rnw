  ...
  \begin{minipage}{13cm}
    {\small
      ...
      {\centering\textbf{Abstract}\\ Replication studies are essential to assess
        ...
        differences in study design and study population, are common in the
...
data sets) or it may only use the replication data. Typically, there will be a
...
different aspects of replicability, and which typically have different success
regions (\eg{} one method for quantifying parameter compatibility and another
for quantifying evidence against a null hypothesis). In this case, the sample
size may be chosen such that the probability of replication success is as large
as desired for all planned analysis methods.
...
$\sigma_{r}$ and integrating the marginal likelihood of the replication effect
...
replication $z$-value $z_{r} = \that_{r}/\sigma_{r}$. In these cases, it is
typically more convenient to use the associated predictive distribution of
$z_{r}$
\begin{align}
  \label{eq:zr}
...
$z_{o} = \that_{o}/\sigma_{o}$, the prior
$z$-value $z_{\scriptscriptstyle{\theta}} = \mutheta/\sigma_{\scriptscriptstyle{\theta}}$,
...
Figure~\ref{fig:data} for a summary. The data were collected in four
laboratories. Each of them conducted four original studies and for each original
study a replication study was carried out within the same lab but also by the
other three labs. % This unique design allows to
% dissentangle between-lab heterogeneity from within-lab variability of effect
% sizes.
...
prior. Using the empirical Bayes approach for the specification of the prior variance
(as described in Section~\ref{sec:shrinkage}), this would lead to a prior with shrinkage factor $\hat{g}/(1 + \hat{g}) = \Sexpr{round(shrinkEBhet, 2)}$ for
an external-replication. The mean and variance would thus be shrunken towards
...
To computing the probability of replication success one needs to select an
analysis methods and integrate the predictive distribution~\eqref{eq:fthetar},
respectively, \eqref{eq:zr} over the associated success region $S$. In the following
we illustrate the success regions of different methods, and show how the
sample size can be computed from them. It is important to note that
...
\begin{align}
  ...
  = \left[z_{\scriptscriptstyle 1 - \alpha}\, \sigma_{r}, \infty \right)
\end{align}
...
\begin{align}
  ...
  = \left[z_{\scriptscriptstyle 1 - \alpha}\, \sigma_{r}
  \left\{\sqrt{1/c + 1} -  z_{o}\sqrt{c}\right\}, \infty \right)
\end{align}
...
\begin{align}
  ...
  = \left[\that_{o} - \Delta - z_{\scriptscriptstyle 1 - \alpha} \sqrt{\sigma^{2}_{o} +
  \sigma^{2}_{r}}, \that_{o} + \Delta - z_{\scriptscriptstyle 1 - \alpha}
...
\begin{align}
  ...
  = \left[\sigma_{r} z_{\scriptscriptstyle 1 - \alpha} \sqrt{1 +
  c/\{(z_{o}^{2}/z_{\scriptscriptstyle 1 - \alpha}^{2}) - 1\}}, \infty\right)
\end{align}
