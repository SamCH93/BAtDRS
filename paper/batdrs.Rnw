...
statistical significance assessment of the replication \citep{Goodman1992,
  Senn2002, Micheloud2020, vanZwet2022}, SSD for reverse-Bayes assessment of
replication studies \citep{Held2020, Pawel2022b}, or SSD for meta-analysis of
...
approaches \citep{Spiegelhalter1986b, Spiegelhalter1986c, Weiss1997,
  ...
  Grieve2022}. Our proposed framework (schematically illustrated in
Figure~\ref{fig:SSDschema}) applies to any kind of data model and analysis
method.
...
prior, is usually desired to be objective or uninformative, in some sense
...
closed-form expression for the predictive distribution of the replication effect
estimate anymore but numerical or Monte Carlo integration need to be used. To
gain more insight, we will thus only consider fixed $\tau^{2}$ in the remaining
part of the paper. In the supplement we illustrate how the probability of
replication success can be computed when a prior is assigned to $\tau^{2}$. The
derived closed-form expression conditional on $\tau^{2}$ are still useful in
this situation as they enable computation of the predictive distribution up to a
single one-dimensional integral which can be computed numerically.
...
\texttt{BayesRepDesign} (Appendix~\ref{app:package}) for all analysis methods
...
The data from original and replication are sometimes pooled via fixed-effects
meta-analysis. The pooled effect estimate $\that_{m}$ and standard error
$\sigma_{m}$ are then given by
\begin{align*}
  &\that_{m} =
...
from above by the limit $\text{limPr}_{\text{2TR}}$ defined
in~\eqref{eq:limP2TR}.
...
goes in opposite direction of the original one, which is know as the
``replication paradox'' \citep{Ly2018}. The paradox can be avoided by using a
modified version of the replication Bayes factor but the success region is then
not available anymore in closed-form \citep[Appendix D]{Pawel2022b}. Based on
the success region~\eqref{eq:BFrsuccess}, the probability of replication success
can be computed by
\begin{align}
  \label{eq:BFrpros}
...
factor'' $\BFs$, and the lower $\BFs$ the higher the degree of replication
success. Also for this method, the success region and the probability of
replication success can be derived in closed-form but the derivations are more
involved than for the other methods. For this reason, they are only given in the
supplement.
...
replication success for meta-analysis. If heterogeneity is taken into account
(middle plot) the probability of replication success becomes closer to 50\% for
all methods but the equivalence test, reflecting the larger uncertainty about
the effect size $\theta$. To achieve $\Sexpr{round(power*100, 2)}\%$ probability
of replication success the fewest samples are required with meta-analysis,
followed by the sceptical $p$-value, the two-trials rule, the replication Bayes
factor, the sceptical Bayes factor, and lastly the equivalence test. If the
chosen sample size should guarantee a sufficiently conclusive replication study
with all these methods, the replication sample size has be slightly larger than
the original one in the situation of no heterogeneity ($\tau^{2} = 0$), while it
has to be more than ten-fold increased if there is heterogeneity
...
into account heterogeneity ($\tau^{2} = \Sexpr{round(tauAbs, 2)}$). The top
...
required to achieve an 80\% target probability with one site (yellow) for the
two-trials rule, whereas only approximately half as many samples are required
for two sites (green).
...
the cost of an additional sample $K_{c}$, \eg{} $K_{S}/K_{c} = 30$ (yellow) the
optimal number of sites is $m = 5$ for the two-trials rule and $m = 8$ for the
replication Bayes factor. If an additional site is more costly the optimal
number of sites is lower, \eg{} if the cost ratio is $K_{S}/K_{c} = 300$
(purple), the optimal number of sites is $m = 2$ for the two trials rule and
$m = 3$ for the replication Bayes factor, while the number of sites in the
actually used by \citet{Protzko2020} was $m = 3$ (counting only
...
proposed) can thus benefit from our methodology. For instance, our methods
