  ...
  \begin{minipage}{13cm}
    {\small
      ...
      {\centering\textbf{Abstract}\\ Replication studies are essential to assess
        ...
        are available. Based on a design prior, predictions about the
...
\begin{align*}
  \that_{r} - \that_{o} \pm \zalpha \sqrt{\sigma^{2}_{r} + \sigma^{2}_{o}}
\end{align*}
...
too small ($\Delta \leq \zalpha \sigma_{o}$), replication success is impossible
regardless of how small the replication standard error $\sigma_{r}$ will be.
Assuming now that the margin is large enough ($\Delta > \zalpha \sigma_{o}$),
...
\begin{align}
  ...
  &= \Phi\left(\frac{\that_{o} + \Delta - \zalpha
...
\begin{align}
  ...
  \Phi\left(\frac{\that_{o} + \Delta - \zalpha\sigma_{o}
...
details. They showed that the success region based on $\BFr \leq \gamma$ is
given by
\begin{align}
  \label{eq:BFssuccess}
...
replication sample sizes based on the previously discussed methods, see
...
plotDF1 <- plotDF %>%
    filter(sp == Inf) %>%
    mutate(facetLab = paste("tau ==", round(tau, 2)),
                                        #paste("theta ~ '~ N(' ~ hat(theta)['o'] * ',' ~ sigma['o']^2 + tau^2 * '),' ~ tau ==", round(tau, 2)),
           order = 1)
plotDFH0 <- plotDF %>%
    filter(sp == 0) %>%
    mutate(facetLab = paste("{italic('H')[0] ~ ':' ~ theta == 0} *',' ~ tau == 0"),
           order = 2)
...
@
\caption{Probability of replication success as a function of relative
  sample size $c = n_{r}/n_{o}$ for experiment ``\Sexpr{studyname}'' with
  original effect estimate $\hat{\theta}_{o} = \Sexpr{round(study[,"dr"], 2)}$
  and standard error $\sigma_{o} = \Sexpr{round(sqrt(study[,"vr"]), 2)}$. Replication
  success is defined by the two-trials rule at level $\alpha = \Sexpr{levelSig}$,
  the replication Bayes factor at level $\gamma = 1/\Sexpr{1/levelBFr}$, fixed
  effects-meta analysis at level $\alpha = \Sexpr{sqrt(levelSigMeta)}^{2}$, effect
  size equivalence at level $\alpha = \Sexpr{1 - confLevel}$ with margin
  $\Delta = \Sexpr{margin}$, sceptical $p$-value at level
  $\alpha = \Sexpr{round(levelPs, 3)}$, and sceptical Bayes factor at
  level $\gamma = 1/\Sexpr{1/levelBFs}$.}
\label{fig:example}
\end{figure}

The left and middle plot in Figure~\ref{fig:example} show the probability of
replication success under the design prior based on a uniform initial prior for
the effect size $\theta$ and data from the original study without (left) and
with (middle) between-study heterogeneity taken into account. It is visible that
the probability increases for monotonically with the relative sample size
for all methods except meta-analysis which.


The right plot show the probability of replication success under the null
hypothesis that the effect size $\theta$ is exactly zero and that there is no
between-study heterogeneity (the design prior~\eqref{eq:dpnormal} with
$\mutheta = 0$, $\sigmatheta^{2} = 0$ and $\tau^{2} = 0$).
...
cBreaks <- c(1/10, 1/3, 1, 3, 10, 30, 100)
...
\caption{The left plot shows the required relative sample size $c = n_r/n_o$ to
  ...
  \citet{Protzko2020}. A flat initial prior for the effect size $\theta$ is used
  along with no heterogeneity ($\tau = 0$) or heterogeneity
  ($\tau = \Sexpr{round(taus[2], 2)}$). The right plot Bottom shows.}
\label{fig:ssd-all}
\end{figure}
