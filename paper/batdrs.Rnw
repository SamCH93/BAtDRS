...
\authornote{
  ...
  drafts of the manuscript. We thank the anonymous reviewer for constructive
  comments. Our acknowledgment of these individuals does not imply their
...
knowledge--- in a so-called \emph{design prior} distribution for the underlying
model parameters. If the analysis of the replication data is also Bayesian, the
design prior may be different from the so-called \emph{analysis prior} which,
unlike the design prior, is usually desired to be objective or ``uninformative''
...
(e.g., one method for quantifying parameter compatibility and another for
quantifying evidence against a null hypothesis). In this case, the sample size
may be chosen such that the probability of replication success is as high as
...
distribution up to a one-dimensional integral which can be computed numerically.
...
arbitrarily high by decreasing the standard error $\sigma_{r}$ but is bounded
from above by $\mathrm{limPr}_{\mathrm{2TR}}$ defined in~\eqref{eq:limP2TR}.
...
values of $\BFs$ pointing to higher degrees of replication success. Also for
this method, the success region and the probability of replication success can
be expressed in closed-form but the derivations are more involved than for the
...
$H_{0} \colon \theta = 0, \tau^{2} = 0$, %, i.e., the effect size
...
The additional samples needed for these studies could thus be reallocated from
the studies that require fewer samples. The project would still use the same
total sample size, but it would be more efficiently allocated.
...
heterogeneity deviation of $\tau = 0.05$ as inputs to the \texttt{designPrior}
function
...
The output shows the relative variance $c = \sigma_{o}/\sigma_{r}$ which,
assuming a standard error form $\sigma_{i} = \lambda/\sqrt{n_{i}}$, is equal to
the relative sample size $c = n_{r}/n_{o}$. The parameter $c$ thus quantifies by
how much the replication sample size $n_{r}$ must be increased/decreased
compared to the original sample size $n_{o}$. The replication standard error can
also be converted to an absolute sample size using

<< echo = TRUE >>=
se2n(se = ssd1$sr, unitSD = 2)
@

By default, the function assumes a unit standard deviation of $\lambda = 2$ for
...
exact conversions can be obtained by considering the exact form of the standard
error and solving for the sample size.
\end{flushleft}

Finally, the BayesRepDesign package can be easily extended to replication
analysis methods other than those for which dedicated functions are provided. To
do so, users need to define a function that returns the success region for the
replication effect estimate for a given replication standard error. The function
is then passed as an argument to the \texttt{ssd} function, which then
numerically determines the required standard error. The following code
illustrates how the significance method from earlier can be reimplemented in
this way.
