...
hierarchical model see][]{Rover2021}. In this case there are is no closed-form
expression for the predictive distribution of the replication effect estimate
anymore but numerical or Monte Carlo integration need to be used. % To
% gain more insight, we will thus only consider fixed $\tau^{2}$ in the remaining
% part of the paper.
In the supplement we illustrate how the probability of replication success can
be computed when a prior is assigned to $\tau^{2}$. The derived closed-form
expression conditional on $\tau^{2}$ are still useful in this situation as they
enable computation of the predictive distribution up to a single one-dimensional
integral which can be computed numerically.
...
for an external-replication. Figure~\ref{fig:dpexample} (dark-grey solid lines)
...
towards zero by $\Sexpr{round((1 - shrinkEBhet)*100, 0)}$\% (medium-grey
short-dashed lines in Figure~\ref{fig:dpexample}). In contrast, if we had prior
...
sum of the precision of both estimates (light-grey long-dashed lines in
...
the replication Bayes factor, while the number of sites in the actually used by
\citet{Protzko2020} was $m = 3$ (counting only external-replications),
respectively, $m = 4$ (counting also the internal-replication).
