...
\documentclass[a4paper, 11pt, man, natbib, floatsintext]{apa7}
...
\abstract{Replication studies are essential for assessing the credibility of
  ...
  application to data from a multisite replication project from Psychology how
  Bayesian approaches help to design informative and cost-effective replication
  studies. Our methods are made available in an R package.}
...
framework for Bayesian SSD of replication studies, %(Section~\ref{sec:GFW}),
followed by investigating design priors and sample size determination closer in
the normal-normal hierarchical model and for several Bayesian and non-Bayesian
analysis methods. %(Section~\ref{sec:SS}).
To illustrate the methodology, we use data from a multisite replication project
...
the ``absolute'' approach as discussed in previously,
...
which is available at \url{https://github.com/SamCH93/BayesRepDesign}. The
