...
In most cases there is also an upper limit on the possible sample size due
to limited resources and/or availability of sample.
...
success corresponds to the ``type I error rate''.

The left and middle plots in Figure~\ref{fig:example} show that increasing the
relative sample size $c$ increases the probability of replication success for
all methods but meta-analysis. Meta-analysis shows a non-monotone behavior
because the original study was already highly significant so that the pooled
effect estimate is significant even for very small replications, and the
uncertainty about the effect estimate from the replication therefore may even
slightly reduce the probability of replication success. To achieve a probability
of replication success of 80\% with all considered methods, the replication
sample size has be slightly larger than the original one in the situation of no
heterogeneity ($\tau^{2} = 0$), while it has to be more than ten-fold increased
if there is heterogeneity ($\tau^{2} = \Sexpr{round(taus[2], 2)}$). However,
this is mostly due to the equivalence test which requires by far the most
samples. If the equivalence test sample size is ignored, the sceptical Bayes
factor requires the most samples among all methods so that
$c = \Sexpr{round(ssbfs2[["c"]], 1)}$ leads to at least 80\% probability of
replication success.


The right plot in~\ref{fig:example} shows the type I error rate of the different
methods as a function of the relative sample size $c$. By definition the type I
error rate of the two-trials rule stays constant at
$\alpha = \Sexpr{round(levelSig, 3)}$. In contrast, the type I error rates of
most other methods stay below the one from the two-trials rule with the
exception being meta-analysis and the sceptical $p$-value. The former has an
extremely highly type I error rate as the pooling with the original data results
in replication success with high probability unless the replication sample size
is drastically larger than the original one. The type I error rate of the
sceptical $p$-value becomes smaller than the one from the two-trials rule if the
replication sample size is approximately three times larger than the original
one.
...
plotA <- ggplot(data = resSSD, aes(x = Studylab, y = c, color = Methodlab,
    ...
    geom_vline(xintercept = seq(1.5, 12.5, 1), lty = 3, alpha = 0.1) +
...
\caption{The left plot shows the required relative sample size $c = n_r/n_o$ to
  ...
  associated with the computed sample size.}
\label{fig:ssd-all}
\end{figure}

Figure~\ref{fig:ssd-all} now shows the required relative sample size $c$ and
associated type I error rates for the other studies from \citet{Protzko2020}.
The studies are ordered with respect to their original $p$-value and only
studies with $p_{o} \leq \Sexpr{levelSig}$ are inclduded since only they can
fulfill the two-trials rule. For all methods but the equivalence test the
required relative sample size $c$ decreases with decreasing original $p$-value.
For the equivalence test the required $c$ depends only on how large the original
standard error $\sigma_{o}$ was, and smaller standard errors require smaller
sample sizes in the replication.
...
<< "package-illustration", echo = TRUE, size = "small" >>=
library("BayesRepDesign")
...
## compute numerically via method agnostic function
## function that returns success region based on replication standard error
sregionfunSig <- function(sr, alpha = 0.025) {
    successRegion(intervals = cbind(stats::qnorm(p = 1- alpha)*sr, Inf))
}
ssd(sregionfun = sregionfunSig, dprior = dp, power = 0.8)
@
