  ...
  \begin{minipage}{13cm}
    {\small
      ...
      {\centering\textbf{Abstract}\\ Replication studies are essential for
        ...
        in other studies. Here we show how Bayesian approaches can be used for
        tackling this problem. The Bayesian framework allows researchers to
        combine the original data and external knowledge in a design prior
        distribution for the underlying parameters. We investigate design priors
        in the normal-normal hierarchical model where analytical results are
        available. Based on a design prior, predictions about the replication
        data can be made, and the replication sample size can be chosen to
        ensure a sufficiently high probability of replication success.
        ...
        approaches. We illustrate with application to data from a multisite
        replication project how Bayesian approaches help to design informative
        and cost-effective replication studies. Our methods are made available
        in an R package.}
      \rule{\textwidth}{0.4pt} \\
...
methodology was developed. For these reasons, a relatively small literature has
emerged which specifically deals with replication study SSD. Apart from
\citet{Anderson2022} who discuss SSD for different analysis methods in a
frequentist framework, most of these papers only deal with selected analysis
methods and data models. For instance, SSD for standardized mean difference
effect sizes analyzed with Bayes factors \citep{Bayarri2002}, SSD for
statistical significance assessment of the replication \citep{Goodman1992,
  Senn2002, Micheloud2020, vanZwet2022}, SSD for reverse-Bayes assessment of the
...
methods under a general framework based on principles from Bayesian design
approaches \citep{Spiegelhalter1986b, Spiegelhalter1986c, Weiss1997,
...
The proposed framework (schematically illustrated in Figure~\ref{fig:SSDschema})
applies to any kind of data model and analysis method. The design of replication
studies is a natural candidate for Bayesian knowledge updating as it allows to
...
from the original study and/or expert knowledge-- in a so-called \emph{design
  prior} distribution for the underlying model parameters. If the analysis of
the replication data is also Bayesian, the design prior may be different from
the so-called \emph{analysis prior} which, unlike the design prior, is usually
desired to be objective or uninformative, in some sense
% , whereas the design prior should represent the resarchers best bet on the
% replication outcome
\citep{OHagan2001b, Gelfand2002}. Based on the design prior, predictions about
the replication data can then be made, and the sample size can be chosen such
that the probability of replication success becomes sufficiently high.
Importantly, Bayesian design approaches can also be used if the planned analysis
of the replication study is non-Bayesian, which is the more common situation in
practice.

We start with presenting a general framework for Bayesian SSD of replication
studies (Section~\ref{sec:GFW}), followed by investigating design priors and
sample size determination closer in the normal normal hierarchical model and for
several Bayesian and non-Bayesian analysis methods (Section~\ref{sec:SS}). To
illustrate the methodology, we use data from a multisite replication project
\citep{Protzko2020} as a running example. We then close with concluding remarks,
limitations, and open questions (Section \ref{sec:discussion}).
...
posterior distribution of $\theta$ based on the original data and an initial
prior for $\theta$
\begin{align}
  \label{eq:dp}
...
replication success is at least as large as a desired target, here and
...
sample size $n_r^*$ may thus also need to satisfy a type I error rate not larger
...
$\sigmatheta^{2}$). In the following, we will explain how these parameters can
be specified based on external knowledge.
...
We note that, in principle, one can also assign a prior distribution to $\tau^2$
\citep[for an overview of prior distributions for heterogeneity variances in the
normal-normal hierarchical model see][]{Rover2021}. In this case there are is no
closed-form expressions for the predictive distribution of the replication
effect estimate anymore but numerical or Monte Carlo integration needs to be
used. To gain more insight, we will thus only consider fixed $\tau^{2}$ in the
remaining part of the paper. In the supplement we illustrate how the probability
of replication success can be computed when a prior is assigned to $\tau^{2}$.
The derived closed-form expression conditional on $\tau^{2}$ are still useful in
this situation as they enable computation of the predictive distribution and
probability of replication success up to two one-dimensional integrals which can
...
absolute value) than the observed one. For instance, replicators could believe
that the results from the original study are exaggerated because there is no
pre-registered study protocol available. Even without such beliefs, weakly
informative shrinkage priors may also be motivated from a ``regularization''
point of view as they can correct for statistical biases \citep{Copas1983,
  Firth1993} or prevent unreasonable parameter values from taking over the
posterior in settings with uninformative data \citep{Gelman2009}.
...
$\Sexpr{round(cio[1], 3)}$ to $\Sexpr{round(cio[2], 3)}$. For each submitted
...
the ``absolute'' approach as discussed in Section~\ref{sec:heterogeneity}, so
that the range between the 2.5\% and the 97.5\% quantile of the study specific
...
success and the required sample size are implemented methods in the R package
\texttt{BayesRepDesign} (Appendix~\ref{app:package}) for all discussed analysis.
...
$n_{r}^{*} \approx 4/(\sigma^*_{r})^{2}$ from Section~\ref{sec:example}.
...
well-known sample size formula
\begin{align*}
...
thus generalizes standard sample size calculation that can take into account the
...
from above by the limit defined in~$\text{limPr}_{\text{2TR}}$
...
\end{align}
for $\Delta \geq \zalphatwo \sqrt{\sigma_{o}^{2} + \sigma^{2}_{r}}$. The success
region~\eqref{eq:equivalence} becomes the empty set if the margin is too small
($\Delta < \zalphatwo \sqrt{\sigma_{o}^{2} + \sigma^{2}_{r}}$) so that
...
($\Delta > \zalphatwo \sqrt{\sigma_{o}^{2} + \sigma^{2}_{r}}$), the probability
...
provided evidence against the null hypothesis, replication Bayes factor values
$\BFr < 1$ indicate replication success, and the smaller $\BFr$ the higher the
degree of replication success. Under normality and assuming no heterogeneity,
the success region for achieving $\BFr \leq \gamma$ is given by
\begin{align}
  \label{eq:BFrsuccess}
...
Details of this calculation are given in the supplement. Interestingly, the
success region~\eqref{eq:BFrsuccess} shows that replication success is also
possible if the replication effect estimate goes in opposite direction of the
original one, which is know as the ``replication paradox'' \citep{Ly2018}. The
paradox can be avoided by using a modified version of the replication Bayes
factor but the success region is then not available anymore in closed-form
\citep[Appendix D]{Pawel2022b}. Based on the success
region~\eqref{eq:BFrsuccess}, the probability of replication success can be
computed by
\begin{align}
  \label{eq:BFrpros}
...
standard error for achieving a target probability of replication success
$1 - \beta < \text{limPr}_{\scriptscriptstyle \BFr}$.
...
normal prior the effect size $\theta$ such that its posterior distribution based
on the original study is no longer credible. Replication success is then
...
the $p$-value the higher the degree of replication success. \citet[Section
...
measure termed the ``sceptical Bayes factor'' $\BFs$, and the lower $\BFs$ the
higher the degree of replication success. The success region and the probability
of replication success can be derived in closed-form but the derivations are
more involved than for the other methods, which is why they are only given in
the supplement.
...
\caption{Probability of replication success as a function of relative sample
  ...
  margin $\Delta = \Sexpr{margin}$, sceptical $p$-value at level
...
even for replication studies with very small sample size \citep{Micheloud2020}.
The uncertainty regarding the replication effect estimate $\that_{r}$ may
therefore even reduce the probability of replication success for meta-analysis.
If heterogeneity is taken into account (middle plot) the probability of
replication success becomes closer to 50\% for all methods but the equivalence
test, reflecting the larger uncertainty about the effect size $\theta$. To
achieve $\Sexpr{round(power*100, 2)}\%$ probability of replication success the
fewest samples are required with meta-analysis, followed by the sceptical
$p$-value, the two-trials rule, the replication Bayes factor, the sceptical
Bayes factor, and lastly the equivalence test. If the chosen sample size should
guarantee a sufficiently conclusive replication study with all these methods,
the replication sample size has be slightly larger than the original one in the
situation of no heterogeneity ($\tau^{2} = 0$), while it has to be more than
ten-fold increased if there is heterogeneity
...
other methods vary with the relative sample size $c$ but most of them stay below
$\alpha = \Sexpr{round(levelSig, 3)}$ for all $c$ with the exception of
meta-analysis and the sceptical $p$-value. Meta-analysis has an extremely high
type I error rate as the pooling with the highly significant original data leads
to replication success if the replication sample size is not drastically
increased. The type I error rate of the sceptical $p$-value is only slightly
larger than $\alpha = \Sexpr{round(levelSig, 3)}$ which is expected since we use
a level $\alpha = \Sexpr{round(levelPs, 3)}$ for declaring replication success
with the sceptical $p$-value, and its type I error rate is always smaller than
the level for thresholding it \citep{Held2020}. The type I error rate of the
sceptical $p$-value decreases to values smaller than
$\alpha = \Sexpr{round(levelSig, 3)}$ at approximately $c = 3$. This
...
\caption{The left plot shows the required relative sample size $c = n_r/n_o$ to
  ...
  rate associated with the computed sample size. Experiments are ordered (top to
...
counterparts. This is still the case if heterogeneity is taken into account in
the design prior (triangles), which generally increases the required sample size
...
realistic and could be reallocated from the other studies which require fewer
...
standard errors are large relative to the specified margin. If one anticipates
to analyze the original and replication pair with an equivalence test, this
should therefore be taken into account already at the design stage of the
original study.
...
estimates are correlated as the covariance matrix in~\eqref{eq:ftrmulti} is
non-diagonal.
...
corresponding success region $S$. This may be more involved for if the success
...
multisite extension of these two methods). The design prior obtained from
updating a uniform initial prior for the effect size ($\sigmatheta \to \infty$)
and taking into account heterogeneity ($\tau^{2} = \Sexpr{round(tauAbs, 2)}$ is
used, as earlier for singlesite SSD. The top plots show the probability of
...
trials rule and $m = 3$ for the replication Bayes factor, whereas the number of
sites in the actual study by \citet{Protzko2020} was $m = 3$ (counting only
...
proposed) can thus benefit from our methodology. For instance, our methodology
could be straightforwardly applied to the ``dual-criterion'' from
\citet{Rosenkranz2021} which defines replication success via simultaneous
statistical significance and clinical relevance of the effect estimates from
...
replicate the conditions from the original study as closely as possible; yet SSD
...
external knowledge can be incorporated. Furthermore, we have looked at the
normal normal hierarchical model with fixed variances in order to obtain
closed-form expressions for the probability of replication success. Specifying
priors on the between-study heterogeneity variances could better reflect the
available uncertainty but would come at the price of lower interpretability and
higher computational complexity. We did also not consider designs where the
replication data are analyzed in a sequential manner. Ideas from the Bayesian
sequential design \citep{Schoenbrodt2017, Stefan2022} or from the adaptive
trials literature \citep{Bretz2009} could be adapted to the replication setting
as in \citet{Micheloud2020}. A sequential analysis of the replication data could
possibly increase the efficiency of the replication. However, it would also make
SSD more challenging. An additional point is that we assumed that the original
study has been completed when planning the replication study. One could also
consider a scenario where both the original and replication study are planned
simultaneously and adopt a ``project'' perspective as in \citet{Held2021}.
However, in this case no information from the original study is available and
the design prior needs to be specified entirely based on external knowledge.
...
effect size which is required to achieve the desired probability of replication
success based on the maximally possible sample size. Researchers can then judge
whether or not such prior beliefs are scientifically sensible, and decide
whether they should conduct the replication study with their limited resources.
