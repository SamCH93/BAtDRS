...
\abstract{Replication studies are essential for assessing the credibility of
  ...
  design prior distribution for the underlying parameters. We investigate design
  priors in the normal-normal hierarchical model where analytical results are
  available. Based on a design prior, predictions about the replication data can
  be made, and the replication sample size can be chosen to ensure a
  sufficiently high probability of replication success. Replication success may
  be defined through Bayesian or non-Bayesian criteria, and different criteria
  may also be combined to meet distinct stakeholders and allow conclusive
  inferences based on multiple analysis approaches. We illustrate with
  application to data from a multisite replication project how Bayesian
  approaches help to design informative and cost-effective replication studies.
  Our methods are made available in an R package.}
...
\authornote{
  ...
  Preprint version \today. % don't forget to hard-code date when submitting to arXiv!
...
in this case be computed. The derived closed-form expression conditional on
...
sum of the precision of both estimates (light-grey dot-dashed lines in
...
\textbf{BayesRepDesign} (Appendix~\ref{app:package}) for all analysis methods
...
replication studies are implemented in the R package \textbf{BayesRepDesign}
...
<< "package-illustration", echo = TRUE, size = "footnotesize", fig.height = 3.5 >>=
library("BayesRepDesign")

## design prior (flat initial prior for effect size + heterogeneity)
dp <- designPrior(to = 0.2, so = 0.05, tau = 0.08)
## plot(dp) # plot design prior with this command
