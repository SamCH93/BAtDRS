...
\authornote{
  ...
  This research has not been pre-registered. A preprint has been previously
  published on arXiv
...
replication success of at least $1 - \beta$, i.e.,
\begin{align}
  \label{eq:nr}
...
example, for the ``\Sexpr{studyname}'' experiment, the design prior would be
...
without heterogeneity ($\tau^{2} = 0$) and with heterogeneity
($\tau^{2} = \Sexpr{round(tauAbs, 2)}^{2}$), respectively. The right plot shows
the prior corresponding to the ``fixed effect null hypothesis''
...
heterogeneity is assumed ($\tau^{2} = 0$), while it has to be increased more
than ten-fold if heterogeneity is assumed
...
which a probability of $\Sexpr{round(power*100, 2)}$\% is achievable, the space
is left blank. For example, in the application of the meta-analysis method to
the ``Labels'' experiment the probability remains above
...
larger than the type I error rate of 2.5\% of the two-trials rule. However,
...
equivalence test may also be large if the replication requires very large sample
...
\citet{Protzko2020} project. For most studies, all methods except the
equivalence test require fewer samples in the replication than in the original
study to achieve a target probability of
$1 - \beta = \Sexpr{round(power*100, 2)}$\% for replication success. This is
still the case when heterogeneity is accounted for (triangles), which generally
...
Some original studies require larger replication sample sizes, and in some cases
these are unrealistically large (e.g., in the ``Prediction'' experiment an
almost 30 times increase in sample size for the replication Bayes factor). In
other cases they seem more realistic and could be reallocated from the other
studies which require fewer samples. On the other hand, the equivalence test
...
target probability with one site %(yellow)
...
the cost of an additional sample $K_{c}$, e.g., $K_{S}/K_{c} = 30$ %(yellow)
...
number of sites is lower, e.g., if the cost ratio is $K_{S}/K_{c} = 300$,
% (purple),
...
standard in meta-analysis, we assumed that the variances of the effect estimates
are known, which can sometimes be inadequate \citep{Jackson2018}. Specifying
priors also for the variances could better reflect the available uncertainty,
but would come at the cost of reduced interpretability and increased
computational complexity. We also did not consider designs in which the
replication data are analyzed sequentially. Ideas from Bayesian sequential
designs \citep{Schoenbrodt2017, Stefan2022} or from adaptive clinical trials
