...
% instance,
\citet{Anderson2016} propose to declare replication success at level
...
\end{align}
Wether or not replication success is achieved depends on the precision of
the original estimate, and in particular when the margin
$\Delta \geq \zalpha \sigma_{o}$, replication success
is impossible regardless of how small the replication standard error $\sigma_{r}$
is. Assuming now that $\Delta < \zalpha \sigma_{o}$, the probability of
...
&\phantom{=} -
  ...
  \tau^{2} + (\sigma^{2}_{o} + \tau^{2})/(1 + 1/g)}}\right),
\end{align}
from which one can see that there can only be a positive probability
if the margin $\Delta$ is large enough relative to the standard error
of the difference $\sqrt{\sigma^{2}_{o} + \sigma^{2}_{r}}$
and the required replication standard error $\sigma_{r}$ to achieve a desired
power of replication success $1 - \beta$ can be computed numerically.
