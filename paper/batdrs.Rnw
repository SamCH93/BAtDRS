...
for quantifying replicability and we will not take part in the discussion
...
Original and replication effect estimates can be pooled via fixed-effects
...
\end{align*}
and are also equivalent to the mean and standard deviation of a posterior
distribution for the effect size $\theta$ based on the data from both studies
and an initial flat prior for $\theta$. The
...
\begin{align}
  ...
  \infty \right)
\end{align}
then corresponds to both replication success defined via a one-sided meta-analytic
...
\tau^{2} + (\sigma^{2}_{o} + \tau^{2})/(1 + 1/g)}}\right).
\end{align}
As for the two-trials rule, by decreasing the standard error $\sigma_{r}$
the probability~\eqref{eq:MApros} can at most become as large as~$\text{limP}_{\text{2TR}}$
from~\eqref{eq:limP2TR}. The required standard error $\sigma_{r}^{*}$
to achieve a desired probability of replication success $1 - \beta < \text{limP}_{\text{2TR}}$
can conveniently be computed using a root finding algorithm.
...
% instance,
\citet{Anderson2016} proposed to declare replication success at level
$\alpha$ if the $(1 - \alpha)$ confidence interval for the effect size difference
$\theta_{r} - \theta_{o}$
\begin{align*}
  \that_{r} - \that_{o} \pm \zalpha \sqrt{\sigma^{2}_{r} + \sigma^{2}_{o}}
\end{align*}
is fully inside an equivalence region $[-\Delta, \Delta]$ defined via the
margin $\Delta > 0$. This procedure corresponds to rejecting the null hypothesis
$H_{0} \colon |\theta_{r} - \theta_{o}| > \Delta$
in an equivalence test, and it implies a success region for the replication effect
...
the original estimate $\sigma_{o}$, and in particular when the margin is chosen too small
($\Delta \leq \zalpha \sigma_{o}$), replication success
is impossible regardless of how small the replication standard error $\sigma_{r}$
will be. Assuming now that $\Delta > \zalpha \sigma_{o}$, the probability of
replication success can be computed by
\begin{align}
  \label{eq:MApros}
...
\end{align*}
which contrasts the likelihood of the replication data $x_{r}$ under the null
hypothesis $H_{0} \colon \theta = 0$ and the alternative
$H_{1} \colon \theta \sim f(\theta \given x_{o})$ which is the posterior of the
effect size $\theta$ given the original data $x_{o}$. Bayes factor values $\BFr < 1$ indicate
replication success. We show in the supplementary that under normality the
...
\end{align}
with $A = \sigma^{2}_{r}(1 + \sigma^{2}_{r}/\sigma^{2}_{o})
\{\that_{o}^{2}/\sigma^{2}_{o} - 2 \log \gamma +
\log(1 + \sigma^{2}_{o}/\sigma^{2}_{r})\}$. Consequently, the probability of
replication success at level $\gamma$ can be computed by
\begin{align}
  \label{eq:BFrpros}
...
based on different sample sizes for the three different analysis approaches. To
specify the maximally tolerable
heterogeneity $\tau^{2}_{\scriptscriptstyle \text{MT}}$ for the EESBF a similar
elicitation approach as in Section~\ref{sec:example} could be applied. For
example, we could specify
$\tau_{\scriptscriptstyle \text{MT}} = \Sexpr{round(tauAbs, 2)}$ as in the
design prior (Figure~\ref{fig:data} uses this value for the analysis of the
actual data). However, the original study is only sufficiently sensitive to
obtain
$\BF_{\text{E}} \leq \sqrt{1 + \tau^{2}_{\scriptscriptstyle \text{MT}}/(\sigma^{2}_{o}/2)} \approx \Sexpr{round(sqrt(1 + tauAbs^2/(0.5*study[,"vr"])), 2)}$
for this $\tau^{2}_{\scriptscriptstyle \text{MT}}$. We therefore choose an
(unrealistic) larger value of
$\tau_{\scriptscriptstyle \text{MT}} = \Sexpr{round(tauMT, 2)}$ so that we can
illustrate SSD of the EESBF for higher levels of evidence.
% for % $\tau^{2}_{\scriptscriptstyle \text{MT}}$
% as the original study was not sufficiently precise the probability of
% replication success would have been zero for
% $\tau_{\scriptscriptstyle \text{MT}} = \Sexpr{round(tauAbs, 2)}$.
% % For the same
% % reason, we choose a lower evidence threshold for the EESBF than for the other
% % two analysis methods.
\begin{figure}[!htb]
<< "example-applied", fig.height = 3.25, eval = FALSE >>=

## parameter of RS methods
levelSig <- 0.025
...
applyGrid <- expand.grid(tau = c(0, tauAbs), DP = DP, stringsAsFactors = FALSE)

plotDF <- do.call("rbind", lapply(X = seq(1, nrow(applyGrid)), FUN = function(i) {
    tau <- applyGrid$tau[i]
    DP <- applyGrid$DP[i]
    plotDF <- rbind(data.frame(c = cseq, tau = tau, DP = DP,
                               pow = powerBFs(level = level, zo = zo, c = cseq,
                                              designPrior = DP,
                                              h = tau^2/study$vr),
                               method = "'BF'['S']"),
                    data.frame(c = cseq, tau = tau, DP = DP,
                               pow = powerBFr(level = level, zo = zo, c = cseq,
                                              designPrior = DP,
                                              h = tau^2/study$vr),
                               method = "'BF'['R']"),
                    data.frame(c = cseq, tau = tau, DP = DP,
                               pow = powerBFee(level = levelBFee, to = study$dr,
                                               so = sqrt(study$vr),
...
plotDF$method <- factor(plotDF$method,
                        levels = c("'BF'['R']", "'BF'['S']", "'BF'['E']"))
plotDF1 <- plotDF %>%
    filter(DP == "predictive") %>%
    mutate(facetLab = paste("tau ==", round(tau, 2)),
           order = 1)
plotDFH0 <- plotDF %>%
    filter(DP == "H0") %>%
    mutate(facetLab = paste("italic('H')[0] ~ ':' ~ theta == 0"),
           order = 2)
...
##     ## geom_hline(yintercept = power, lty = 3, alpha = 0.3) +
##     ## geom_hline(yintercept = 0.025, lty = 1, alpha = 0.1) +
...
##     labs(x = ## bquote("Relative variance" ~ italic(c) == sigma[italic(o)]^2/sigma[italic(r)]^2),
##              bquote("Relative sample size" ~ italic(c) == italic(n[r]/n[o])),
...
## tau <- 0
## DP <- "predictive"
## ssbfs <- ssBFs(level = level, power = power, zo = zo, h = tau^2/study$vr,
##                designPrior = DP)
## ssbfr <- ssBFr(level = level, power = power, zo = zo, h = tau^2/study$vr,
##                designPrior = DP)
## ssbfee <- ssBFee(level = levelBFee, power = power, to = study$dr,
##                  so = sqrt(study$vr), tauMT = tauMT, designPrior = DP,
##                  tau = tau)

## tau <- tauAbs
## DP <- "predictive"
## ssbfs2 <- ssBFs(level = level, power = power, zo = zo, h = tau^2/study$vr,
##                 designPrior = DP)
## ssbfr2 <- ssBFr(level = level, power = power, zo = zo, h = tau^2/study$vr,
##                 designPrior = DP)
## ssbfee2 <- ssBFee(level = levelBFee, power = power, to = study$dr,
##                   so = sqrt(study$vr), tauMT = tauMT, designPrior = DP,
##                   tau = tau)
@
\caption{Probability of replication success as a function of relative
  ...
  and standard error $\sigma_{o} = \Sexpr{round(sqrt(study[,"vr"]), 2)}$.}
\label{fig:example}
\end{figure}
