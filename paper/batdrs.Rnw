...
\abstract{Replication studies are essential for assessing the credibility of
  ...
  design prior distribution for the underlying parameters. We investigate sample
  size determination for design priors in the normal-normal hierarchical model
  where analytical results are available. In this framework no Monte Carlo
  simulation is required, and frequentist sample size determination is a special
  case where the uncertainty from the original data is ignored. Based on a
  design prior, predictions about the replication data can be made, and the
  replication sample size can be chosen to ensure a sufficiently high
  probability of replication success. Replication success may be defined through
  Bayesian or non-Bayesian criteria, and different criteria may also be combined
  to meet distinct stakeholders and allow conclusive inferences based on
  multiple analysis approaches. We illustrate with application to data from a
  multisite replication project in Psychology how Bayesian approaches help to
  design informative and cost-effective replication studies. Our methods are
  available in the R package BayesRepDesign.}
...
\authornote{
  ...
  Preprint version
  \today. % don't forget to hard-code date when submitting to arXiv!
...
\citep{Bayarri2002, Goodman1992, Senn2002, Micheloud2020, vanZwet2022, Held2020,
  Pawel2022b, Hedges2021}. However, most of these articles only deal with
selected analysis methods and data models. % For
...
An exception is the excellent article from \citet{Anderson2022} who discuss more
general principles of replication SSD in the context of psychological research,
mostly from a frequentist perspective. As they state ``the literature on
Bayesian sample size planning is still nascent, particularly with respect to
...
based on principles from Bayesian design approaches \citep{Spiegelhalter1986b,
  ...
  Spiegelhalter2004, Schoenbrodt2017, Pek2019,
...
``Bayesian assurance'' \citep{OHagan2005}, and has also been used for
psychological applications \citep{Pek2019}.
...
model framework which provides flexibility for incorporating the original data
and external knowledge in replication design. %(Section~\ref{sec:SS}).
...
project in psychology \citep{Protzko2020}. Finally, we close with concluding
remarks, limitations, and open questions. %(Section \ref{sec:discussion}).
...
\begin{figure}[!htb]
<< "analyse-protzko", fig.height = 8.75 >>=
...
\caption{Design priors for the effect size $\theta$ (SMD) in the experiment
  ...
  the effect size $\theta$: uninformative corresponding to a flat prior for
  $\theta$, shrinkage corresponding to a zero-mean normal prior for $\theta$
  with empirical Bayes variance estimate~\eqref{eq:EBestimate}, and optimistic
  corresponding to a flat prior updated by the data from a pilot study with
  effect estimate $\hat{\theta}_{p} = \Sexpr{round(extstudy[,"dr"], 3)}$ and
  standard error $\sigma_{p} = \Sexpr{round(sqrt(extstudy[,"vr"]), 3)}$.}
\label{fig:dpexample}
\end{figure}
