...
test. % This is still
% the case when heterogeneity is accounted for (triangles), which generally
% increases the required sample size compared to when heterogeneity is not
% accounted for (points), especially for studies with large original $p$-values.
Some original studies were less convincing and therefore require larger
replication sample sizes. % In some cases these are unrealistically large (e.g.,
% in the ``Prediction'' experiment an almost 30 times increase in sample size for
% the replication Bayes factor).
% In other cases they seem more realistic and could
% be reallocated from the other studies which require fewer samples.
...
A key strength of the approach is that it can be applied to any type of
replication analysis method, Bayesian or non-Bayesian, as long as there is a
...
approaches in the normal-normal hierarchical model (or not even proposed) can
thus benefit from our methodology. For instance, our methods could easily be
...
plot(dp)
## compute replication standard error to achieve one-sided p-value <= 2.5%
ssdSig(level = 0.025, dprior = dp, power = 0.8)
## compute replication standard error to achieve replication Bayes factor <= 1/10
ssdBFr(level = 1/10, dprior = dp, power = 0.8)
@
