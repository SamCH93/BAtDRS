...
\abstract{Replication studies are essential for assessing the credibility of
  ...
  where analytical results are available and traditional sample size
  determination is a special case where the uncertainty from the original data
  is ignored. Based on a design prior, predictions about the replication data
  can be made, and the replication sample size can be chosen to ensure a
  ...
  application to data from a multisite replication project in Psychology how
  Bayesian approaches help to design informative and cost-effective replication
  studies. Our methods are available in the R package BayesRepDesign.}
...
\authornote{
  ...
  Preprint version November 4,
  2022. % don't forget to hard-code date when submitting to arXiv!
  ...
  Charlotte Micheloud and Angelika Stefan for helpful comments on drafts of the
  manuscript. Our acknowledgment of these individuals does not imply their
  endorsement of this article.

  The CC-By 4.0 licensed data were downloaded from
  \href{https://osf.io/42ef9/}{https://osf.io/42ef9/}. Analyses were conducted
  in the R programming language version \Sexpr{paste(version[["major"]],
    version[["minor"]], sep = ".")} \citep{R}. Code to reproduce this manuscript
  is available at
  ...
  \href{https://doi.org/10.5281/zenodo.XXXXXX}{https://doi.org/10.5281/zenodo.X}.
...
based on principles from Bayesian design approaches \citep{Spiegelhalter1986b,
  ...
  Spiegelhalter2004, Schoenbrodt2017, Pek2019, Park2022,
...
original data and external knowledge in replication design. %(Section~\ref{sec:SS}).
...
followed by the skeptical $p$-value, the two-trials rule, the replication Bayes
factor, the skeptical Bayes factor, and lastly the equivalence test. If the
