  \begin{tikzpicture}[thick, node distance = 11em and 1em, font=\small
    % , font=\sffamily
    ]
  ...
  \node[rectangle, draw, rounded corners = 0.2em] (external) [right = 22em of oData]
 ...
 \node (rData) [below = 3em of designPrior]
 {\begin{tabular}{c} \textbf{Predictive distribution of replication data} \\
...
%     % {\scriptsize -- Parameter estimation \dots}
     %    \end{tabular}};
\node[rectangle, draw, rounded corners = 0.2em] (measureRS) [below = 11em of oData]
...
{\begin{tabular}{l} \textbf{Side constraints} \\
   ...
   {\scriptsize -- precision requirements} \\
   {\scriptsize -- cost/availability limitations} \\
   {\scriptsize $\phantom{\rightarrow} \hdots$} \\
 \end{tabular}};
...
\draw [->] [bend right, dashed] (external.west) to ([xshift=1em]designPrior.north);
